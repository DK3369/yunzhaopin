    ...
    <div class="moduleElHight">
        ...
        <table class="tableVue">
            <thead>
                <tr align="left">
                    ...
                    <th width="260">{{ lc('admin_tool_00536') }}</th>
                        ...
                        <div v-show="temptype == '1'">
                            ...
                            <div class="tw_jobscbox">
                                <input id="twcomcopy" data-clipboard-action="copy"
                                    data-clipboard-target="#twcomtemp_content" type="button" value=lc('admin_tool_00526')
                        ...
                        <div v-show="temptype == '0'">
                            ...
                            <div class="tw_jobscbox">
                                <input type="button" class="admin_Filter_bth" id="twcomgzh_copy"
                                    data-clipboard-action="copy" data-clipboard-target="#twcomgzh_content" value=lc('admin_tool_00526') />
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <div class="TableSelect">
                            <el-select v-model="tempid" size="small" slot="prepend" :placeholder="lc('admin_tool_00545')">
                    ...
                    <td>
                        <div class="TableSelect">
                            <el-select v-model="com_search" @visible-change="comChoosed" multiple filterable remote
                                reserve-keyword :placeholder="lc('admin_tool_00544')" :remote-method="comSearch"
                        ...
                        <div class="tool_inputips"><i class="el-icon-question"></i> {{ lc('admin_tool_00534') }} <el-button type="text"
                                @click="clearCom">{{ lc('admin_tool_00542') }}</el-button></div>
        ...
        <div class="moduleSeachs" style="padding-top:10px;">
            <div class="moduleSeachleft" style="display: flex; flex-wrap: wrap; align-items: center;">
                ...
                <div class="tableSeachInpt" style="margin-bottom: 0px;;">
                    <el-input :placeholder="lc('admin_tool_00529')" style="width: 300px;" clearable v-model="keyword"
                        prefix-icon="el-icon-search">
                    </el-input>
                </div>
                <div class="tableSeachInpt" style="margin-bottom: 0px;;">
                    <el-select v-model="auid" size="small" slot="prepend" :placeholder="lc('admin_tool_00546')" clearable @change="search">
                        <el-option v-for="admin in adminList" :key="admin.uid" :label="admin.name ? admin.name : admin.username" :value="admin.uid"></el-option>
                    </el-select>
                </div>

                <div class="tableSeachInpt" style="margin-bottom: 0px;;">
                    <el-select v-model="status" size="small" slot="prepend" :placeholder="lc('member_user_00181')" @change="search" clearable>
        ...
        <div class="moduleElTable">
            <el-table ref="table" :data="tableData" v-loading="list_loading" @selection-change="selectionChange"
                ...
                <el-table-column :label="lc('admin_user_company_00159')">
                ...
                <el-table-column :label="lc('wap_com_00157')">
                    <template slot-scope="scope">
                        <el-link type="primary" :href="scope.row.comurl" target="_blank">{{ scope.row.comname }}</el-link>
                    </template>
                </el-table-column>

                <el-table-column prop="jobsdate_n" :label="lc('admin_tool_00530')"></el-table-column>
                ...
                <el-table-column prop="admin_username" :label="lc('admin_tool_00531')"> </el-table-column>
                ...
                <el-table-column :label="lc('member_user_00048')" width="300" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="success" size="mini" @click="addcom(scope.row.id, scope.row.cuid, scope.row.comname)"
                            :disabled="scope.row.comstatus == '2' || com_uids.indexOf(scope.row.cuid) != -1">{{ lc('wap_00215') }}</el-button>
                        <el-button type="primary" size="mini" @click="taskFinish(scope.row.id)"
                            :disabled="scope.row.status == '1'">{{ lc('admin_01270') }}</el-button>
        ...
        <div class="modulePaging">
            <div class="modulecz">
                ...
                <el-button size="mini" @click="multiAdd">{{ lc('admin_tool_00533') }}</el-button>
                <el-button size="mini" @click="multiFinish">{{ lc('admin_user_company_00376') }}</el-button>
            </div>
    ...
    methods: {
        ...
        multiFinish: function () {
            ...
            if (this.choosedata.length > 0) {
                ...
                message.error(window.yunAdminT(lc('admin_user_weipin_00001'))); return;
            }
            var params = {
                id: idarr
            };
        ...
        async getTW() {
            ...
            if (this.tempid == '') {
                message.error(window.yunAdminT(lc('admin_tool_00548'))); return;
            }
            if (this.com_uids.length == 0) {
                message.error(window.yunAdminT(lc('admin_tool_00549'))); return;
            }
