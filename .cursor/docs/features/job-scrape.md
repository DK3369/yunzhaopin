...
| 写入 | **可见正文（去标签）≥ 100 字才 INSERT**。不足或只有 Company/Location/`Apply / source` 占位的不写 `phpyun_company_job`。`compose_description` 清洗后约 60KB；**禁止** `job::repo::update`（会把 `state=0`）；回填用 `update_description_keep_listed`。删占位岗或有新增/更新后 `home_service::invalidate_all`，避免首页 60 秒缓存还链到已删 ID | |
