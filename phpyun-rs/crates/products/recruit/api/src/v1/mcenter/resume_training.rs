//! Training experience CRUD (usertype=1). Delete is folded into update (`status: 2`).
...
use phpyun_models::resume::training::{Training, TrainingInput};
use phpyun_services::resume_children_service::training_svc;
...
impl From<Training> for TrainingItem {
    fn from(r: Training) -> Self {
        Self {
            id: r.id,
            uid: r.uid,
            eid: r.eid,
...
#[utoipa::path(
    post,
    path = "/v1/mcenter/resume/trainings",
    tag = "mcenter",
    security(("bearer" = [])),
    request_body = TrainingForm,
...
pub async fn create(
    ...
    ValidatedJson(f): ValidatedJson<TrainingForm>,
) -> AppResult<ApiResponse<CreatedId>> {
    let id = training_svc::create(
        &state,
        &user,
        TrainingInput {
            name: &f.name,
...
#[utoipa::path(
    post,
    path = "/v1/mcenter/resume/trainings/update",
    tag = "mcenter",
    security(("bearer" = [])),
    request_body = TrainingForm,
    responses((status = 200, description = "ok"))
...
pub async fn update(
    ...
    if f.status == Some(2) {
        training_svc::delete(&state, &user, f.id, &ip).await?;
