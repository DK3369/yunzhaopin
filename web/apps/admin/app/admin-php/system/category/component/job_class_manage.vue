<template>
    <div class="moduleElenAlCategorySub">
        <div class="moduleElTable moduleElTableCategoreSub">
            <el-table :data="tableData" stripe border style="width: 100%;" height="100%"
                ...
                <el-table-column header-align="center" align="right" :label="lc('member_user_00048')" :width="210">
                    <template #default="scope">
                        <div class="cz_button">
                            <el-button v-if="scope.row.level == 'two'" size="small" @click="moveRow(1, scope)">{{ lc('admin_system_00115') }}</el-button>
                            <el-button v-if="scope.row.level == 'three'" size="small" @click="moveRow(2, scope)">{{ lc('admin_system_00115') }}</el-button>
                            <el-button v-if="scope.row.level == 'two' || scope.row.level == 'three'" size="small" @click="editRow(scope)">{{ lc('wap_js_00073') }}</el-button>
                            <el-button size="small" @click="deleteRow(scope)" type="danger">{{ lc('common.delete') }}</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <div class="modluDrawer">
            <el-drawer :title="titleAddEdit" v-model="addVisible" :modal-append-to-body="false" :append-to-body="true" :show-close="true"
                :destroy-on-close="true" size="80%">
                <job_class_edit :tid="tid" :id="threeid" @child-event-getlist="getList"></job_class_edit>
            </el-drawer>
        </div>
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_system_00117')" v-model="moveVisible" :with-header="true" :modal-append-to-body="false" :append-to-body="true"
...
<script>
import JobClassEdit from './job_class_edit.vue'
