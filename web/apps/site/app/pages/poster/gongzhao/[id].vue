...
const { t } = useI18n()
const api = useApi()
const { data: tpls } = await useAsyncData('gz-poster-tpls', () =>
  api.post<Array<{ id: number; title: string; pic?: string }>>('/v1/wap/posters/templates', { kind: 'gongzhao' }).catch(() => []),
)
const hb = computed(() => Number(route.query.hb || 0) || undefined)
const { data: spec } = await useAsyncData(
  () => `gz-poster-${id}-${hb.value || 0}`,
  () => api.post('/v1/wap/posters', { kind: 'gongzhao', id, hb: hb.value }).catch(() => null),
)
