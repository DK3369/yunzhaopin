...
const { data: vip, error, refresh: refreshVip } = await useAsyncData('user-vip-orders', () =>
  api.post<{ list: OrderRow[] }>('/v1/mcenter/vip/orders/list', { page: 1, page_size: 50 }).catch(() => ({ list: [] })),
)
const { data: redeem, refresh: refreshRedeem } = await useAsyncData('user-redeem-orders-merge', () =>
...
const merged = computed<OrderRow[]>(() => {
  ...
  return [...a, ...c].sort((x, y) => Number(y.created_at || 0) - Number(x.created_at || 0))
})
...
async function cancelOrder(o: OrderRow) {
  ...
  try {
    ...
    await Promise.all([refreshVip(), refreshRedeem()])
  } catch (e: unknown) {
  ...
  <MemberPanel :title="$t('common_02029')" :error="error && !isUnauthErr(error) ? error : undefined">
    ...
    <div v-for="o in paged" :key="(o.kind || '') + (o.order_no || o.id)" class="site-pc paylist_list">
      ...
      <span class="paylist_span paylist_cz">
        <NuxtLink v-if="canPay(o)" :to="`/user/cashier/${o.order_no}`" class="cblue">{{ $t('wap_00401') }}</NuxtLink>
        ...
        <ul>
          <li v-for="o in paged" :key="'h5-' + (o.kind || '') + (o.order_no || o.id)">
            ...
            <div class="detail_box_cz">
              <NuxtLink v-if="canPay(o)" :to="`/user/cashier/${o.order_no}`">{{ $t('wap_00401') }}</NuxtLink>
