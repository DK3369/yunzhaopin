...
class once_controller extends common{
	function index_action(){
		...
		$this->get_moblie();
		$this->seo("once");
		$this->yunset("topplaceholder",yun_at('wap_00334'));
		$this->yunset("headertitle",yun_at('wap_js_00130'));
	...
	function add_action(){ 
		...
		$this->yunset("headertitle",yun_at('wap_js_00130'));
		$this->yunset("title",yun_at('wap_01792'));
	...
	function show_action(){
		...
		if($row['status']<1  && !$_GET['pay']){
			$this->ACT_msg_wap(Url('wap',array('c'=>'once')),yun_at('wap_00337'), 1, 3);
			
		}elseif($row['pay']=='1' && !$_GET['pay']){
			$this->ACT_msg_wap(Url('wap',array('c'=>'once')),yun_at('wap_00336'), 1, 3);
		}
		$this->yunset("row",$row);
	...
	function pay_action(){
		...
		if($_GET['id']){
            if(empty($_GET['oncepricegear'])){
                $this->ACT_msg_wap(Url('wap',array('c'=>'once')),yun_at('wap_01793'), 1, 3);
            }
			
			if(!$row){
				$this->ACT_msg_wap(Url('wap',array('c'=>'once')),yun_at('wap_00335'), 1, 3);
			}
	...
	function paylog_action(){
		...
		$this->yunset("headertitle",yun_at('default_00031'));
		$this->seo('once');
