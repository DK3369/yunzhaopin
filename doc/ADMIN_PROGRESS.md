...
> 2026-08-30 · 分支 `feat/frontend-backend-split` · 当时 HEAD `bc694820`  
...
- `phpMap`：具名 `m/c/a` + `MODULE_ROUTES` 的 list/save/del/status。具名 action **禁止**静默落到 list（`904fff4b`）。
- 刻意不做：校园/猎头/培训/spview、`database`/`generate_*`/`admin_uc`。

## Rust 后台 API

- Crate：`phpyun-rs/crates/products/recruit/api-admin`（约 48 个 `v1/*.rs`）。
- OpenAPI 快照仍锁 **297** 条（`doc/snapshots/admin_paths.txt`）。PHP 同构 `php-*` **不进 AdminDoc**。
- 已按 PHP 补：企业校验/开户/编辑/Imitate/套餐/comcert、职位 add、简历/会员 add-edit-save、SEO/注册设置/短信/海报形状、公告 add GET 表单。

## 伪删除

提交 `9e129225`：

1. 二次校验：路径以 `/delete`、`/purge` 结尾时，再查 `phpyun_admin_user` 是否有效。
2. 白名单约 39 张表 `deleted=1`；列表 `COALESCE(deleted,0)=0`。
3. 迁移 `phpyun-rs/migrations/sqlx/20260829000001_admin_soft_delete.sql`。

仍用物理 DELETE 或业务状态位：职位 `state=2`、会员/企业注销、KV/银行卡/海报模板、日志/回收站 purge、**招聘会主表 `phpyun_zhaopinhui`（未进白名单，只有展位表）**。
...
| 招聘会 / 新闻 / 问答 / 专题具名 action | 进行中（见下） |
| 前台 OAuth 回调 + 支付真单 | 未做（排在后台内容写路径之后） |

## 仍弱或故意不做

- RBAC 不解析 `group_power`；导出是 CSV 不是 OLE xls
- 微信自定义菜单能力仍薄
- 支付回调 handler 在，沙箱真单未验收
