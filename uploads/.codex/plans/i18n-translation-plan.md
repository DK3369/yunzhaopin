...
## Admin finance runtime repair batch

This batch is limited to `app/template/admin/yunying/caiwu/chongzhidd.html` and the admin dashboard bootstrap in `app/template/admin/index/index/index.html`. Restore the finance order contract-status expression that was corrupted by nested Vue interpolation during i18n migration. Keep the condition in Vue and translate only the two pure status labels through existing keys. Defer Element UI table scrolling until the table ref is mounted and guard the ref before access. Stop injecting the unavailable `init.ov6.com` script so a dead optional remote service cannot add repeated connection errors to admin pages. Mark the legacy page scripts with `data-cfasync="false"` so Cloudflare Rocket Loader does not reorder their Vue bootstrap dependencies. Define the finance page's `[v-cloak]` rule in `<head>` before external scripts so raw Vue interpolation is hidden on the first paint, then let Vue reveal the page after mounting. Do not change finance API routes, request fields, order calculations, pagination semantics, or existing language-pack values in this batch.

## Admin Vue first-paint batch

This batch covers the shared admin bootstraps `app/template/admin/header.js` and `app/template/admin/home_header.js`, the admin shell include in `app/template/admin/index.htm`, and the exact `header.js` include tag in the 110 admin HTML pages. The shared bootstrap must hide the body before it can paint raw Vue interpolation, show a text-free loading spinner, and reveal with a short fade only after a root Vue 2 instance is mounted. Use an eight-second fallback release so a broken page is never hidden permanently. Mark only the shared header include with `data-cfasync="false"`; this guarantees the guard runs in `<head>` even when Cloudflare Rocket Loader handles later page scripts. Add an explicit deployment version query to the shared header URL because these static files are cached publicly for 24 hours. The HTML migration is restricted to those two attributes on the exact shared-header script tag and must not alter page markup, Vue expressions, language keys, APIs, or business behavior.
