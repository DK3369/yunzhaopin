...
**最近一次更新**: 2026-04-28（Round 27 Model 收敛 —— 合并 `sysmsg` ↔ `message` 两个同根 `phpyun_sysmsg` 模块，删除 deprecated 的 `/v1/mcenter/sys-messages*` 4 个路由 + `sysmsg_service` + `sysmsg` model；`message_repo` 新增 `delete_by_ids` / `insert_simple` helper 接管原 sysmsg 的批量删与一行插入。53 个静态测试通过）  
**下一步**: Round 28+ 处理仍待裁决的同根 model 簇 —— `phpyun_yqmb`（interview / interview_template / invite，需要先收敛数据库 schema）、`phpyun_company_order`（once_job / redeem / vip 共用支付订单表，需要抽统一 `order` 模块 + `payment_kind` 鉴别字段）
