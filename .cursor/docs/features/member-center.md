...
| H5 首页宫格 | 简历、隐私、其他服务、财务、**最后一项账户设置**。意见反馈只在 `/user/set`，**不要**再占「我的」宫格。退出在 `/user/set`，不要把退出当宫格最后一项。右侧「更多」只在有摘要时出现（简历「去完善」、其他服务兼职/问答） | 企业资料、财务、其他服务，**最后一项账户设置**。退出在 `/com/set`。其他服务摘要用 `wap_com_00089` |
