...
async fn audit_underlying_table_counts() {
    ...
    let probes: &[(&str, &[(&str, &str)])] = &[
        ...
        (
            ...
            &[("phpyun_evaluate_log", "uid = 1")],
        ),
        (
            "/v1/mcenter/my/questions",
            &[("phpyun_question", "uid = 1")],
        ),
    ...
    for (path, tables) in probes {
        ...
        for (t, cond) in *tables {
            ...
            let result: Result<(i64,), sqlx::Error> = sqlx::query_as(&sql).fetch_one(db).await;
