...
const LC_FIRST_WINS: Record<WebLocale, Record<string, string>> = {
  zh: {
    ...
    admin_pay_methods_hint: 'Stripe 可真收。GCash / PayMaya 本轮仅配置，点支付提示未接入。密钥只保存在服务器。',
    admin_pay_merchants_title: '支付商户',
    admin_pay_merchants_hint: 'OV6 自用。外部商户用 HMAC 调 /v1/pay/orders。Secret 只在创建或轮换时显示一次。',
  ...
  en: {
    ...
    admin_pay_methods_hint:
      'Stripe charges live. GCash / PayMaya are config-only this round; pay returns not connected. Secrets stay on the server.',
    admin_pay_merchants_title: 'Merchants',
    admin_pay_merchants_hint:
      'OV6 is built-in. External merchants HMAC POST /v1/pay/orders. Secret is shown once on create or rotate.',
