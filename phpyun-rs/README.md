...
本地另起进程时才 `cargo run -p phpyun-rs`（须改 `BIND`）。契约：`http://127.0.0.1:3003/docs/`（Swagger，仅 dev/test）、`/api-docs/v1/openapi.json`。细节见 [`.cursor/docs/rust/run.md`](../.cursor/docs/rust/run.md)。
...
`/v1/wap` 与 `/v1/mcenter` 是 Flutter App 的线上契约，**只允许加法**。公开读接口另挂了 GET 别名供 SSR/CDN。PC/H5 用 `/v1/wap/initjobs`、`/v1/wap/initads`；单条 `dict` / `countries` / `ads?slot=` 仅 App 契约，勿删。
