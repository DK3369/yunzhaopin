...
- 收银台 [`MemberCashier`](../../../web/layers/ui/app/components/MemberCashier.vue) `/com/cashier/[order_no]` `/user/cashier/[order_no]`：`POST /orders/detail` `/orders/pay`。渠道读 **支付网关** OV6 商户 `active` methods（种子 Stripe 开、GCash/PayMaya/PayPal/GrabPay 暂停占位）。`pay` 先写 `phpyun_rs_pay_order` 再开 Stripe Hosted Checkout（`req_customer_email` 须一致才复用 Session）。非 Stripe 即使启用也返回 `not_configured`。Checkout `customer_email` 用会员公开邮箱，`.local`/`.test` 等测试域回落到 `sy_webemail`。当前页跳 `checkout.stripe.com`。`success_url`/`cancel_url` 落公开 [`/pay/stripe`](../../../web/apps/site/app/pages/pay/stripe.vue)，带 `session_id` 登录后调 `POST /v1/mcenter/orders/stripe-return`。webhook `POST /callback/stripe` 与 `POST /callback/pay/stripe`。对账：网关表 `phpyun_rs_pay_order` + Stripe 表 `phpyun_rs_stripe_order`。密钥只在 method `config_json`，不读 `sy_stripe_sk`。后台 **支付** 菜单管概览/订单/方式/商户/回调，**不要**改 System 支付设置页。详见 [pay-gateway.md](./pay-gateway.md)。
