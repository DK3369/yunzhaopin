...
pub async fn create_code(pool: &MySqlPool, company_uid: u64, code: &str, note: &str, max_uses: u32, expires_at: i64, now: i64) -> Result<u64, sqlx::Error> {
    let r=sqlx::query("INSERT INTO phpyun_rs_company_invite_codes(company_uid,code,note,max_uses,expires_at,created_at) VALUES(?,?,?,?,?,?)")
        .bind(company_uid).bind(code).bind(note).bind(max_uses).bind(expires_at).bind(now).execute(pool).await?; Ok(r.last_insert_id())
}
pub async fn find_code_active(pool: &MySqlPool, code: &str, now: i64) -> Result<Option<InviteCode>, sqlx::Error> {
...
pub async fn list_codes(pool: &MySqlPool, company_uid: u64) -> Result<Vec<InviteCode>, sqlx::Error> {
...
pub async fn add_hr(pool: &MySqlPool, company_uid: u64, hr_uid: u64, role: &str, now: i64) -> Result<(), sqlx::Error> {
    sqlx::query("INSERT INTO phpyun_rs_company_hrs(company_uid,hr_uid,role,joined_at,status) VALUES(?,?,?,?,1) ON DUPLICATE KEY UPDATE role=VALUES(role),status=1")
        .bind(company_uid).bind(hr_uid).bind(role).bind(now).execute(pool).await?; Ok(())
}
pub async fn remove_hr(pool: &MySqlPool, company_uid: u64, hr_uid: u64) -> Result<u64, sqlx::Error> {
    Ok(sqlx::query("UPDATE phpyun_rs_company_hrs SET status=0 WHERE company_uid=? AND hr_uid=? AND status=1")
        .bind(company_uid).bind(hr_uid).execute(pool).await?.rows_affected())
}
...
pub async fn list_companies_for_hr(pool: &MySqlPool, hr_uid: u64) -> Result<Vec<CompanyHr>, sqlx::Error> {
