    ...
    <div class="moduleElHight">
                ...
                <div class="moduleInptList" style="margin-bottom: 8px;">
                    <el-input placeholder="{yun:}t key='admin_user_weipin_00003'{/yun}" @keyup.enter.native="handleSearch" size="small" v-model="searchForm.keyword" class="input-with-select" clearable>
                        <el-select v-model="searchForm.type" slot="prepend" placeholder="{yun:}t key='wap_user_00100'{/yun}">
                            <el-option label="{yun:}t key='admin_user_00265'{/yun}" value="1"></el-option>
                            <el-option label="{yun:}t key='admin_user_00264'{/yun}" value="2"></el-option>
                            <el-option label="{yun:}t key='wap_com_00157'{/yun}" value="3"></el-option>
                        </el-select>
                    </el-input>
                </div>
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="{yun:}t key='admin_company_00019'{/yun}" start-placeholder="{yun:}t key='admin_00523'{/yun}" end-placeholder="{yun:}t key='admin_00524'{/yun}" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column prop="com_name" label="{yun:}t key='wap_com_00157'{/yun}" min-width="200" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-link :href="scope.row.com_url" target="_blank" type="primary">{{ scope.row.com_name }}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="username" label="{yun:}t key='admin_vue_00025'{/yun}">
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.username }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="resume_name" label="{yun:}t key='admin_user_00264'{/yun}" min-width="150" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.resume_name }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="datetime" label="{yun:}t key='member_user_00197'{/yun}" width="150" sortable="custom">
