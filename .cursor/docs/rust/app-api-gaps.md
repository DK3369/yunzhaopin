...
招聘端后来补的 `zph/cancel`、`com-stats/trend|…`、`com-tongji`、`company/nav`、`sub-accounts`、收银台 `orders/detail|/pay`、`zph/order`、`com-parts` 等见 [member-center.md](../features/member-center.md)，**不要把本文当招聘端总清单**。
...
- **没有** `/orders/overview`。客户端自己合并：`vip/orders/list` + `redeem/orders` + `packs/orders/list` + `once-jobs/paylogs`。收银台再支付是 `orders/detail` + `/orders/pay`（见 member-center）。
