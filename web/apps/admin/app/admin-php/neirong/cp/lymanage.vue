...
<div id="hyfl" class="moduleElenAl">
			...
			<div class="moduleInptList">
			    <el-input :placeholder="lc('admin_00340')" v-model="keyword" class="input-with-select" clearable>
			        <template #prepend><el-select v-model="type" :placeholder="lc('admin_user_00140')">
			            ...
			            <el-option :label="lc('admin_00128')" value="2"></el-option>
			        </el-select></template>
			    </el-input>
			</div>
    ...
    <div class="moduleElTable">
        <el-table :data="tableData" stripe border style="width: 100%;height: 100%;"
            ...
            <el-table-column prop="content" :label="lc('admin_00128')">
                <template #default="scope">
                    <span v-if="scope.row.message.length < 50">{{scope.row.message}}</span>
                    <el-popover v-else trigger="hover" placement="right">
                        <p>{{scope.row.message}}</p>
                        <template #reference><div class="name-wrapper">
                            <el-link>{{ lc('admin_00127') }}<i class="el-icon-view el-icon--right"></i></el-link>
                        </div></template>
                    </el-popover>
                </template>
            </el-table-column>
            <el-table-column :label="lc('admin_00129')" property="ctime_n" width="150">
            </el-table-column>
...
export default {
        ...
        methods: {
            ...
            async getList() {
                ...
                httpPost('m=neirong&c=evaluate&a=message', params, {hideloading: true}).then(function (result) {
                    var res = result.data
                    if (res.error == 0) {
            ...
            async delete(id) {
                ...
                httpPost('m=neirong&c=evaluate&a=delmsg', params).then(function (response) {
