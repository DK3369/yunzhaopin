...
const api = useApi()
const { t } = useI18n()
const { data, error, refresh } = await useAsyncData('com-iv', () =>
...
<template>
  <MemberPanel :title="$t('wap_user_00216')" :error="error" :empty="false">
    <article v-for="row in data?.list || []" :key="row.id" class="look_resume_list">
      <p>
        <NuxtLink :to="`/resumes/${row.uid}`">{{ row.uid }}</NuxtLink>
        · {{ $t('common.job') }} {{ row.job_id }} · {{ row.status_n }}
      </p>
      <p class="muted">{{ row.datetime_n || row.inter_time_n }} · {{ row.address }}</p>
    </article>
