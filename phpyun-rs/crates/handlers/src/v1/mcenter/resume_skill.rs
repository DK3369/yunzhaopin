...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/resume/skills", get(list).post(create))
        .route("/resume/skills/{id}", post(update))
}
...
pub async fn list(
    ...
    Ok(ApiJson(
        list.into_iter()
            .map(|s| SkillItem::from_with_dict(s, &dicts))
            .collect(),
    ))
}
...
#[utoipa::path(
    post,
    path = "/v1/mcenter/resume/skills/{id}",
    tag = "mcenter",
    security(("bearer" = [])),
    params(("id" = u64, Path)),
    request_body = SkillForm,
...
pub async fn update(
    ...
    if f.status == Some(2) {
        skill_svc::delete(&state, &user, id, &ip).await?;
