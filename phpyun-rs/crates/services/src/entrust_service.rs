...
pub async fn count_mine(state: &AppState, user: &AuthenticatedUser) -> AppResult<u64> {
...
pub async fn bind(state: &AppState, user: &AuthenticatedUser, lt_uid: u64) -> AppResult<u64> {
    user.require_jobseeker()?;
    if lt_uid == 0 {
    ...
    if let Some(existing) = entrust_repo::find_binding(state.db.reader(), user.uid, lt_uid).await? {
...
pub async fn unbind(state: &AppState, user: &AuthenticatedUser, lt_uid: u64) -> AppResult<()> {
    user.require_jobseeker()?;
...
pub async fn unbind_by_id(state: &AppState, user: &AuthenticatedUser, id: u64) -> AppResult<()> {
    user.require_jobseeker()?;
