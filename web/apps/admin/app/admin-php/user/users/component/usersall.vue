    ...
    <div class="moduleElHight" :class="searchClass == 'drawer' ? 'pad_lr_20' : ''">
        <div class="moduleSeachbig">
            <div class="tableSeachInpt tableSeachInptsmall" style="padding: 2px 0;">
                <el-input v-model="searchForm.keyword" @keyup.enter="search" :placeholder="lc('admin_user_00158')" size="small"
                    clearable>
                    <template #prepend><el-select v-model="searchForm.type" size="small" :placeholder="lc('admin_user_00140')" style="padding-left:20px;">
            ...
            <div class="tableSeachInpt">
                <el-button type="primary" plain icon="el-icon-document-add" size="small" @click="openAdd">{{ lc('admin_user_00305') }}</el-button>
            </div>
            <div class="tableSeachzk" :class="{ 'searchbutnKai': seachbutn }" style="margin-bottom: 11px;">
                <el-button type="info" class="zhankai" @click="seachbutn = !seachbutn, tableHig = !tableHig"
                    aria-disabled="false" size="small" plain>{{ lc('admin_user_00145') }}
                    <i class="el-icon-arrow-down el-icon--right"></i>
                </el-button>
                <el-button type="info" class="shouqi" @click="seachbutn = !seachbutn, tableHig = !tableHig"
                    aria-disabled="false" size="small" plain>{{ lc('admin_user_00144') }}
        ...
        <div class="moduleElTable" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="list" style="width: 100%" stripe ref="multipleTable" @selection-change="handleSelectionChange"
                    ...
                    <template #default="scope">
                        ...
                        <div class="rz_box">
                            ...
                            <el-tooltip v-if="scope.row.wxid != '' || scope.row.wxopenid != ''"
                                ...
                                <div v-html="lc('admin_user_company_00129') + '<br/>' + scope.row.wxBindmsg"></div>
                                <el-button type="text">
                                    <i class="rzicon rzicon_wxyrz"></i>
                                </el-button>
                            </el-tooltip>
                            <el-tooltip v-else class="item" effect="dark" placement="top-start">
                                <div v-html="lc('member_user_00504') + '<br/>' + scope.row.wxBindmsg"></div>
                                <el-button type="text">
                ...
                <el-table-column :label="lc('member_user_00048')" width="80" fixed="right" align="center">
                    <template #default="scope">
                        <div class="cz_button">
                            <el-button size="small" plain @click="openDetail(scope.$index, scope.row)">{{ lc('member_com_00380') }}</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <div class="modulePaging" style="height: initial; flex-wrap: wrap; padding-top: 10px;">
            <div  style="width:100%;">
                ...
                <el-button @click="batch('auth')" size="small">{{ lc('admin_user_00292') }}</el-button>
            </div>
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_00547')" v-model="dialogDel" :with-header="true" append-to-body :show-close="true"
                width="300px">
                <div>
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('wap_01030')" v-model="dialogIdcardRz" :with-header="true" :modal-append-to-body="false"
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('member_com_00071')" v-model="dialogMoblieRz" :with-header="true" :modal-append-to-body="false"
                :show-close="true" width="450px">
                <div>
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('wap_com_00186')" v-model="dialogEmailRz" :with-header="true" :modal-append-to-body="false"
                :show-close="true" width="450px">
                <div>
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_00548')" v-model="dialogAdd" :append-to-body="true" width="450px">
                <div>
                    ...
                    <el-input @mousedown="pwdMousedown" @input="pwdchange" @focus="readonlyCtl(false)" @blur="readonlyCtl(true)" :readonly="pwdreadonly" :placeholder="lc('wap_00703')" v-model="ruleFormAdd.password" ></el-input>
            ...
            <el-dialog :title="lc('admin_user_weipin_00029')" v-model="dialogDomain" append-to-body :show-close="true" width="500px">
            ...
            <el-dialog :title="lc('admin_user_00292')" v-model="dialogAuth" append-to-body :show-close="true" width="500px">
                <div class="toolClasDia fenpeizhand">
                    <div class="toolClasList">
                        <div class="toolClasTite">
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_user_00280')" v-model="dialogAccountMerge" :with-header="true" :modal-append-to-body="false"
        ...
        <!-- Delete dialog -->
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_00549')" v-model="scdrawer" :with-header="true" :modal-append-to-body="false"
                :show-close="true" width="450px" append-to-body>
                {{ lc('admin_00538') }}
                <template #footer><span class="dialog-footer">
        ...
        <div class="modluDrawer">
            <el-dialog :title="lc('admin_user_00191')" v-model="dialogAccount" :with-header="true" :modal-append-to-body="false"
                :show-close="true" width="450px" append-to-body>
                <div>
                    <div class="wxsettip_small ">{{ lc('admin_user_00140') }}</div>
        ...
        <el-drawer :title="lc('admin_user_00291')" v-model="drawerDetail" @closed="closedDetail" :modal-append-to-body="false" size="95%"
            :append-to-body="true">
            <div class="shbox">
                <div class="shinfo">
                    <div class="sh_zwsz">
                        <el-button type="primary" size="small" @click="toMember(detail)"><i class="el-icon-school"></i>
                    ...
                    <div class="shcomtel" style="padding-bottom:15px; padding-top:10px;;border:none;font-size: 13px;">
                        ...
                        <div class="cominfocz">
                            <el-button type="primary" @click="openAccount" size="small">
                                <i class="el-icon-edit"></i>{{ lc('admin_user_00191') }}
                            </el-button>
                            <el-button type="primary" @click="openAccountMerge" size="small">
                                <i class="el-icon-document-add"></i>{{ lc('admin_00542') }}
                            </el-button>
                            <el-button type="primary" @click="resetPassword(detail)" size="small">
                                <i class="el-icon-thumb"></i>{{ lc('admin_user_00137') }}
                            </el-button>
                            <el-button type="primary" size="small" @click="openDomain(resume)">
                                <i class="el-icon-map-location"></i>{{ lc('admin_user_weipin_00029') }}
                            </el-button>
                            <el-button type="primary" @click="openDel(index)" size="small">
                        ...
                        <el-tab-pane :label="lc('admin_00550')" name="resume">
                            <div v-loading="expectLoading">
                                ...
                                <div class="user_resume_list">
                                    ...
                                    <div class="shshow_p">
                                        <el-tag size="small" v-for="(tagItem, tagIndex) in resume.arrayTag" :key="tagIndex">
                                            {{ tagItem }}
                                        </el-tag>
            ...
            <el-drawer :title="lc('admin_00475')" :append-to-body="true" v-model="drawerBasic" :wrapper-closable="false"
                size="60%">
                                ...
                                <td>
                                    <div class="TableInpt">
                                        <el-input v-model="ruleFormBasic.height" :placeholder="lc('admin_00479')"
                                            @input="inputFloatNumber($event, 'ruleFormBasic', 'height')"><template #append><template
                                               >CM</template></template></el-input>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <div class="TableInpt">
                                        <el-input v-model="ruleFormBasic.weight" :placeholder="lc('admin_00480')"
                                            @input="inputFloatNumber($event, 'ruleFormBasic', 'weight')"><template #append><template
                                               >KG</template></template></el-input>
                                    </div>
                                </td>
                            </tr>
                            <tr>
        ...
        <!-- Add resume -->
        <div class="modluDrawer">
            <el-drawer :title="lc('admin_user_00193')" v-model="drawerResume" append-to-body :wrapper-closable="false" size="45%">
