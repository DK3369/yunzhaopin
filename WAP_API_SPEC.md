...
**最近一次更新**: 2026-04-27（Round 26 DTO 收敛 —— 把 14 个 handler-local body 结构体抽到 `phpyun_core::dto::{IdBody, StatusFilterBody, KindTargetUidBody, IdPasswordBody}`；同时在 dto.rs 标注 `{id, status}` / `{ids, status}` / `{limit}` 这些**故意不抽**的形状（contract bound 各 handler 不同）。53 个静态测试全部通过）  
**下一步**: Round 27+ 扩展到 model / service 层（`crates/models/src/` 244 文件、`crates/services/src/` 104 文件审计是否存在重叠 entity / repo 调用面），以及 mcenter 路由层细分（jobseeker-only / employer-only 中间件，对照 admin guard 模式扩展）
